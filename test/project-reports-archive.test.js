  ...
  const total = db.prepare(
    `SELECT
      (SELECT COUNT(*) FROM project_reports
        WHERE project_id = ? AND deleted_at IS NULL AND archived_at IS NULL)
      + (SELECT COUNT(*) FROM project_failure_reports
        WHERE project_id = ? AND archived_at IS NULL)
      AS c`,
  ).get(project.id, project.id).c;
  assert.equal(total, tech + failure);
});
